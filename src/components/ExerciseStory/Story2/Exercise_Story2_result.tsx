...
export default class Exercise_Story2 extends React.Component<ScreenProps, ScreenState> {
  ...
  async result() {
    ...
    await AsyncStorage.getItem('squart_miss2', (err, result) => {
      if (result == null) {
        squart_miss = 0
      } else {
    ...
    await AsyncStorage.getItem('standingside_miss2', (err, result) => {
      if (result == null) {
        standingside_miss = 0
      } else {
    ...
    await AsyncStorage.getItem('tree_miss2', (err, result) => {
      if (result == null) {
        tree_miss = 0
      } else {
    ...
    await this.state.soundObject9.loadAsync(require('../../../resource/story2/war.mp3'));
    await this.state.soundObject9.setStatusAsync({ volume: 0.1})
    ...
    if (this.state.result == 'success') {
      if( this.props.navigation.isFocused()){
        ...
        await this.state.soundObject4.setStatusAsync({ volume: 0.9})
