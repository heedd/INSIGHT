...
export default class Exercise_Story1 extends React.Component<ScreenProps, ScreenState> {
  ...
  async result() {
    ...
    /////////////////////////////합불//////////////////////////////
    //chest_miss <= 5 && standingside_miss <= 5 && squart_miss <= 5 && sum >= 200
    if (sum >= 250) {
    ...
    await this.state.soundObject4.loadAsync(require('../../../resource/story1/_Beautiful_Korea.mp3'));
    ...
    if (this.state.result == 'success') {
      if( this.props.navigation.isFocused()){
        ...
        await this.state.soundObject.loadAsync(require('../../../resource/story1/story1_success.mp3'));
      ...
      if( this.props.navigation.isFocused()){
        await this.delay(500)
        await this.state.soundObject.loadAsync(require('../../../resource/story1/story1_fail.mp3'));
        await this.state.soundObject.playAsync();
        await this.delay(5000);}
    ...
    const final_fail =
      <ImageBackground
        ...
        <View style={styles.mainContainer}>
          <Text style={{ fontSize: width * 0.13, textAlign: 'center', fontWeight: 'bold' }}>도전 결과</Text>
          <Text style={{ fontSize: width * 0.08, textAlign: 'center', marginTop: 50, marginBottom: height * 0.01 }}>제 1과목 : {`${this.state.tree_score}`}</Text>
          <Text style={{ fontSize: width * 0.08, textAlign: 'center', marginTop: 20, marginBottom: height * 0.01 }}>제 2과목 : {`${this.state.squart_score}`}</Text>
          <Text style={{ fontSize: width * 0.08, textAlign: 'center', marginTop: 20, marginBottom: height * 0.01 }}>제 3과목 : {`${this.state.standingside_score}`}</Text>
          <Text style={{ fontSize: width * 0.083, fontWeight: 'bold', textAlign: 'center', marginTop: 20, marginBottom: height * 0.05 }}>
          {this.state.complete2?'총점 : '+(this.state.tree_score + this.state.squart_score + this.state.standingside_score)+'점':''}</Text>
          <Text style={{ fontSize: width * 0.13, textAlign: 'center', fontWeight: 'bold' }}>{this.state.complete?'실패':''}</Text>
        </View>
      </ImageBackground>
    ...
    const final_success =
      <ImageBackground
        ...
        <View style={styles.mainContainer}>
          ...
          <Text style={{ fontSize: width * 0.083, fontWeight: 'bold', textAlign: 'center', marginTop: 20, marginBottom: height * 0.05 }}>
          {this.state.complete2?'총점 : '+(this.state.tree_score + this.state.squart_score + this.state.standingside_score)+'점':''}</Text>
            <Text style={{ fontSize: width * 0.13, textAlign: 'center', fontWeight: 'bold' }}>{this.state.complete?'합격':''}</Text>
        </View>
      </ImageBackground>
