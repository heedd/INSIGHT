...
export default class ChestOpener extends React.Component<ScreenProps, ScreenState> {
  rafID?: number;
  ...
  async loadtmModel() {
    ...
    const modelJson = 'http://34.86.103.226:8000/chestopener_model';
    const metaJson = 'http://34.86.103.226:8000/chestopener_metadata';
  ...
  async init(
    ...
    const loop = async () => {
      ...
      //피드백 알고리즘
      if (prediction[0].probability > 0.8 && pose != undefined && keypoints.length >= 7) {
        if (status == 2) {
          count++
          total_count++
          history++
      ...
      else if (prediction[1].probability > 0.8 && pose != undefined && keypoints.length >= 7) {
        status = 2
      }
    ...
    const soundObject = new Audio.Sound();
    await soundObject.loadAsync(require('../../resource/exercise/exercise_chest1.mp3'));
    await soundObject.playAsync();
  ...
  componentWillUnmount() {
    ...
    AsyncStorage.setItem('ChestOpener', total_count.toString(), () => { });
