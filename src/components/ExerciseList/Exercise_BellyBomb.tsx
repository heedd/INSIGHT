...
export default class BellyBomb extends React.Component<ScreenProps, ScreenState> {
  rafID?: number;
  ...
  //동작분류 모델 load
  async loadtmModel() {
    const modelJson = 'http://34.86.103.226:8000/bellybomb_model';
    const metaJson = 'http://34.86.103.226:8000/bellybomb_metadata';
  ...
  async init(
    ...
    const loop = async () => {
      ...
      //피드백 알고리즘
      if (prediction[0].probability > 0.70 && pose != undefined && keypoints.length >= 7) {
        if (status == 2) {
          count++
          total_count++
          history++
      ...
      else if (prediction[1].probability > 0.70 && pose != undefined && keypoints.length >= 7) {
        if (status == 1) {
          status = 2
        }
      }
      else if (prediction[2].probability > 0.70 && pose != undefined && keypoints.length >= 7) {
    ...
    const soundObject = new Audio.Sound();
    await soundObject.loadAsync(require('../../resource/exercise/exercise_belly1.mp3'));
    await soundObject.playAsync();
  ...
  componentWillUnmount() {
    ...
    AsyncStorage.setItem('BellyBomb', total_count.toString(), () => { });
  ...
  UNSAFE_componentWillMount() {
    ...
    soundObject16.loadAsync(require('../../resource/exercise/exercise_belly2.mp3'));
  }
