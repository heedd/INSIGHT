...
export default class SideBomb extends React.Component<ScreenProps, ScreenState> {
  ...
  async loadtmModel() {
    ...
    const modelJson = 'http://34.86.103.226:8000/sidebomb_model';
    const metaJson = 'http://34.86.103.226:8000/sidebomb_metadata';
  ...
  async init(
    ...
    const loop = async () => {
      ...
      if (prediction[0].probability > prediction[1].probability && prediction[0].probability > prediction[2].probability) {
        ...
        error++
        if (error > 4 && pose != undefined && keypoints.length >= 7) {
    ...
    const soundObject = new Audio.Sound();
    await soundObject.loadAsync(require('../../resource/exercise/exercise_side1.mp3'));
    await soundObject.playAsync();
  ...
  componentWillUnmount() {
    ...
    AsyncStorage.setItem('SideBomb', total_count.toString(), () => { });
  ...
  UNSAFE_componentWillMount() {
    ...
    soundObject16.loadAsync(require('../../resource/exercise/exercise_side2.mp3'));
  }
