...
export default class standingside extends React.Component<ScreenProps, ScreenState> {
  ...
  async standingside_init(
    ...
    const loop = async () => {
      ...
      if (standingside_count == 8) {
        ...
        await soundObject.loadAsync(require('../../resource/exercise/exercise_squart8.mp3'));
      ...
      //피드백 알고리즘
      if (prediction[0].probability >= 0.60 && pose != undefined && keypoints.length >= 7) {
        standingside_status = "stand"
      }
      else if (prediction[1].probability >= 0.99 && pose != undefined && keypoints.length >= 7) {
      ...
      else if (prediction[2].probability >= 0.50 && pose != undefined && keypoints.length >= 7) {
      ...
      else if (prediction[3].probability == 1.00 && pose != undefined && keypoints.length >= 7) {
    ...
    const soundObject = new Audio.Sound();
    await soundObject.loadAsync(require('../../resource/exercise/exercise_standing1.mp3'));
    await soundObject.playAsync();
  ...
  UNSAFE_componentWillMount() {
    ...
    soundObject1.loadAsync(require('../../resource/exercise/exercise_squart6.mp3'));

    const soundObject5 = new Audio.Sound();
    this.setState({ soundObject5 })
    soundObject2.loadAsync(require('../../resource/exercise/exercise_squart7.mp3'));

    const soundObject6 = new Audio.Sound();
    this.setState({ soundObject6 })
    soundObject3.loadAsync(require('../../resource/exercise/exercise_standing4.mp3'));

    const soundObject7 = new Audio.Sound();
    this.setState({ soundObject7 })
    soundObject1.loadAsync(require('../../resource/exercise/exercise_standing5.mp3'));

    const soundObject8 = new Audio.Sound();
    this.setState({ soundObject8 })
    soundObject2.loadAsync(require('../../resource/exercise/exercise_standing6.mp3'));

    const soundObject9 = new Audio.Sound();
    this.setState({ soundObject9 })
    soundObject3.loadAsync(require('../../resource/exercise/exercise_standing3.mp3'));

    const soundObject10 = new Audio.Sound();
    this.setState({ soundObject10 })
    soundObject3.loadAsync(require('../../resource/exercise/exercise_standing2.mp3'));
  }
